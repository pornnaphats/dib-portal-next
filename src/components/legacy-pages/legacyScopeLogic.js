const getWorkloadColor = (hours) => {
  ...
  window.renderScopeTableRows = function(data, days) {
    ...
    return data.map(group => {
      ...
      <tr class="modern-row">
        ...
        ` : ''}
        <td class="scope-name-cell" style="padding: 14px 20px; border-bottom: 1px solid var(--border); border-right: 1px solid var(--border); background: var(--surface); position: sticky; left: ${colOffset}px; z-index: 10; box-shadow: var(--shadow)">
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px">
            <div style="font-size: 0.85rem; font-weight: 600; color: var(--text-2)">${item.name}</div>
            <div class="row-actions" style="display: flex; align-items: center; gap: 4px; opacity: 0; transform: translateX(5px); transition: all 0.2s ease;">
              <button class="btn-icon" title="แก้ไข" onclick="showEditWorkshipScopeModal('${group.account.replace(/'/g, "\\'")}', '${item.name.replace(/'/g, "\\'")}', '${item.node}', ${item.progress})" style="width: 28px; height: 28px; padding: 0; border-radius: 50%; background: var(--primary-light); color: var(--primary); border: none; display: flex; align-items: center; justify-content: center; cursor: pointer;">
                <i data-lucide="edit-3" style="width: 14px; height: 14px"></i>
              </button>
              <button class="btn-icon" title="ลบ" onclick="deleteWorkshipScope('${group.account.replace(/'/g, "\\'")}', '${item.name.replace(/'/g, "\\'")}')" style="width: 28px; height: 28px; padding: 0; border-radius: 50%; background: rgba(239, 68, 68, 0.1); color: #ef4444; border: none; display: flex; align-items: center; justify-content: center; cursor: pointer;">
        ...
        <td style="padding: 8px 12px; text-align: center; border-bottom: 1px solid var(--border); border-right: 1px solid var(--border); background: var(--surface); position: sticky; left: ${420 + colOffset}px; z-index: 10; box-shadow: var(--shadow)">
          <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%;">
            <div style="font-size: 0.75rem; font-weight: 700; color: ${(() => {
              ...
              if (p <= 120) return '#15803d'; // Dark Green
              return '#991b1b'; // Dark Red
            })()}; margin-bottom: 4px;">${item.progress}%</div>
            <div style="width: 80px; height: 8px; background: #e2e8f0; border-radius: 99px; overflow: hidden; position: relative;">
              <div style="width: ${Math.min(item.progress, 100)}%; height: 100%; background: ${(() => {
                const p = item.progress;
                if (p < 50) return '#ef4444'; // Red (<50%)
                if (p <= 80) return '#eab308'; // Yellow (50-80%)
                if (p <= 100) return '#22c55e'; // Green (81-100%)
                if (p <= 120) return '#15803d'; // Dark Green (101-120%)
        ...
        return `
            <td style="padding: 12px; text-align: center; border-bottom: 1px solid var(--border); border-right: 1px solid var(--border); background: ${cellBg}">
              ${displayNames ? `<div style="font-size: 0.75rem; font-weight: 700; color: ${accStyle.text}; display: inline-block">${displayNames}</div>` : '<span style="color: #e2e8f0">-</span>'}
  ...
  window.showAddWorkshipScopeModal = function () {
    ...
    <div id="addScopeModal" style="position: fixed; inset: 0; background: rgba(15,23,42,0.3); backdrop-filter: blur(8px); z-index: 9999; display: flex; align-items: center; justify-content: center; font-family: 'Kanit', sans-serif;">
      <div style="background: white; width: 480px; border-radius: 24px; box-shadow: var(--shadow); overflow: hidden; transform: translateY(0); transition: all 0.3s">
        <div style="padding: 24px 32px; background: var(--surface2); border-bottom: 1px solid var(--border); color: var(--text); display: flex; justify-content: space-between; align-items: center">
          <h2 style="margin: 0; font-size: 1.1rem; font-weight: 700; color: var(--text); font-family: Kanit">Add New Scope</h2>
          <button onclick="document.getElementById('addScopeModal').remove()" style="background: none; border: none; cursor: pointer; color: var(--text-3); display: flex; align-items: center; justify-content: center">
        ...
        <div style="padding: 32px; display: flex; flex-direction: column; gap: 20px">
          ...
          <div>
            ...
            <div style="display: flex; gap: 8px">
              <select id="scopeAccount" onchange="toggleNewAccountInput(this.value); this.style.color = (this.value && this.value !== 'NEW') ? window.colorForProject(this.value) : '';" style="flex: 1; height: 34px; padding: 0 20px; border: 1.5px solid #e2e8f0; border-radius: 99px; font-family: inherit; outline: none; background: #f8fafc; font-size: 13px; font-weight: 600; transition: color 0.2s;">
              ...
              <input id="newAccountInput" type="text" placeholder="Enter new project name" style="display: none; flex: 1.5; height: 34px; padding: 0 20px; border: 1.5px solid #6366f1; border-radius: 99px; font-family: inherit; outline: none; font-size: 13px;">
            </div>
          </div>
          ...
          <div>
            ...
            <select id="scopeNode" style="width: 100%; height: 34px; padding: 0 20px; border: 1.5px solid #e2e8f0; border-radius: 99px; font-family: inherit; outline: none; background: #f8fafc; font-size: 13px;">
          ...
          <div>
            ...
            <input id="scopeDetail" type="text" placeholder="e.g., Social Monitoring Plan" style="width: 100%; height: 34px; padding: 0 20px; border: 1.5px solid #e2e8f0; border-radius: 99px; font-family: inherit; outline: none; font-size: 13px;">
          </div>
          ...
          <div>
            ...
            <div style="display: flex; align-items: center; gap: 12px">
              <input id="scopePercent" type="number" value="0" min="0" max="1000" style="width: 80px; height: 34px; padding: 0 16px; border: 1.5px solid #e2e8f0; border-radius: 99px; font-family: inherit; outline: none; text-align: center; font-size: 13px;">
          ...
          <button onclick="saveNewWorkshipScope()" class="btn btn-primary" style="margin-top: 12px; width: 100%; height: 42px; display: flex; align-items: center; justify-content: center; border-radius: 99px; font-weight: 700; font-family: inherit; font-size: 14px; cursor: pointer; box-shadow: 0 4px 12px rgba(99,102,241,0.2);">
            Save Scope
          </button>
        </div>
      </div>
  ...
  window.showEditWorkshipScopeModal = function (acc, name, node, progress) {
    ...
    const modalHtml = `
    <div id="addScopeModal" style="position: fixed; inset: 0; background: rgba(15,23,42,0.3); backdrop-filter: blur(8px); z-index: 9999; display: flex; align-items: center; justify-content: center; font-family: 'Kanit', sans-serif;">
      <div style="background: white; width: 100%; max-width: 500px; border-radius: 24px; box-shadow: var(--shadow); overflow: hidden; position: relative;">
        <!-- Header -->
        <div style="padding: 24px 32px; background: var(--surface2); border-bottom: 1px solid var(--border); color: var(--text); display: flex; justify-content: space-between; align-items: center">
          <div>
            <h2 style="margin: 0; font-size: 1.1rem; font-weight: 700; color: var(--text); font-family: Kanit">Edit Scope Details</h2>
            <p style="margin: 4px 0 0; font-size: 0.8rem; color: var(--text-3)">Update workload and details</p>
          </div>
          <button onclick="document.getElementById('addScopeModal').remove()" style="background: none; border: none; cursor: pointer; color: var(--text-3); display: flex; align-items: center; justify-content: center">
            <i data-lucide="x" style="width: 20px; height: 20px"></i>
          </button>
        ...
        <div style="padding: 32px; display: flex; flex-direction: column; gap: 24px">
          ...
          <div>
            <label style="display: block; font-size: 0.85rem; font-weight: 700; color: #475569; margin-bottom: 8px">Project</label>
            <select id="scopeAccount" onchange="this.style.color = window.colorForProject(this.value);" style="width: 100%; height: 34px; padding: 0 20px; border: 1.5px solid #e2e8f0; border-radius: 99px; font-family: inherit; outline: none; background: #f8fafc; font-size: 13px; font-weight: 600; color: ${typeof window.colorForProject === 'function' ? window.colorForProject(acc) : '#6366f1'}; transition: color 0.2s;">
              ${accounts.map(a => {
                const c = typeof window.colorForProject === 'function' ? window.colorForProject(a) : '#6366f1';
          ...
          <div>
            <label style="display: block; font-size: 0.85rem; font-weight: 700; color: #475569; margin-bottom: 8px">Scope / Details</label>
            <input id="scopeDetail" type="text" value="${name}" style="width: 100%; height: 34px; padding: 0 20px; border: 1.5px solid #e2e8f0; border-radius: 99px; font-family: inherit; outline: none; font-size: 13px;">
          </div>
          ...
          <div>
            <label style="display: block; font-size: 0.85rem; font-weight: 700; color: #475569; margin-bottom: 8px">Workload (%)</label>
            <div style="display: flex; align-items: center; gap: 12px">
              <input id="scopePercent" type="number" value="${progress}" min="0" max="1000" style="width: 80px; height: 34px; padding: 0 16px; border: 1.5px solid #e2e8f0; border-radius: 99px; font-family: inherit; outline: none; text-align: center; font-size: 13px;">
          ...
          <div style="display: flex; gap: 12px; margin-top: 8px">
            <button onclick="deleteWorkshipScope('${acc.replace(/'/g, "\\'")}', '${name.replace(/'/g, "\\'")}')" class="btn btn-danger" style="flex: 1; height: 42px; padding: 0 14px; border: 1.5px solid #fee2e2; border-radius: 99px; background: #fef2f2; color: #ef4444; font-weight: 700; font-family: inherit; font-size: 14px; cursor: pointer; display: flex; align-items: center; justify-content: center;">Delete</button>
            <button onclick="document.getElementById('addScopeModal').remove()" class="btn btn-outline" style="flex: 1; height: 42px; padding: 0 14px; border: 1.5px solid #e2e8f0; border-radius: 99px; background: white; color: #64748b; font-weight: 700; font-family: inherit; font-size: 14px; cursor: pointer; display: flex; align-items: center; justify-content: center;">Cancel</button>
            <button onclick="saveNewWorkshipScope(true)" class="btn btn-primary" style="flex: 2; height: 42px; padding: 0 14px; border: none; border-radius: 99px; background: #6366f1; color: white; font-weight: 700; font-family: inherit; font-size: 14px; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 6px -1px rgba(99, 102, 241, 0.4);">Update Scope</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div style="padding:24px 20px; border-bottom:1px solid var(--border); background: var(--surface); position:sticky; top:0; z-index:10; box-shadow: var(--shadow)">
        ...
        <div style="display:flex; flex-direction:column; gap:8px">
          <div style="position:relative">
            ...
            <input type="text" id="sidebarSearch" placeholder="Search..." onkeyup="filterSidebarTasks()" value="${window._sidebarSearch}"
