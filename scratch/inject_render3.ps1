        ...
        <div class="card" style="padding:20px; overflow-x:auto;">
            ...
            <table style="width:100%; border-collapse:collapse; min-width:800px;">
                <thead style="background:#f8f9fb;">
                    <tr>
                        <th style="padding:12px; text-align:left; font-size:.75rem; color:var(--text-3); font-weight:700;">รหัสพนักงาน</th>
                        <th style="padding:12px; text-align:left; font-size:.75rem; color:var(--text-3); font-weight:700;">ชื่อ-นามสกุล</th>
                        <th style="padding:12px; text-align:center; font-size:.75rem; color:var(--text-3); font-weight:700;">มาทำงาน</th>
                        <th style="padding:12px; text-align:center; font-size:.75rem; color:var(--text-3); font-weight:700;">สาย (นาที)</th>
                        <th style="padding:12px; text-align:center; font-size:.75rem; color:var(--text-3); font-weight:700;">ขาดงาน</th>
                        <th style="padding:12px; text-align:center; font-size:.75rem; color:var(--text-3); font-weight:700;">ป่วย/กิจ/พักร้อน</th>
                    </tr>
                </thead>
                <tbody>
                    ${report.map(r => `
                        <tr style="border-bottom:1px solid #f1f5f9;">
                            <td style="padding:12px; font-size:.8rem; font-weight:600; color:var(--text-2);">${r.id}</td>
                            <td style="padding:12px; font-size:.8rem; color:var(--text-1);">${r.name}</td>
                            <td style="padding:12px; text-align:center; font-size:.8rem; color:var(--text-2);">${r.present}</td>
                            <td style="padding:12px; text-align:center; font-size:.8rem; font-weight:600; color:${r.lateMins>0?'#F59E0B':'var(--text-3)'}">${r.lateMins}</td>
                            <td style="padding:12px; text-align:center; font-size:.8rem; font-weight:600; color:${r.absent>0?'#EF4444':'var(--text-3)'}">${r.absent}</td>
                            <td style="padding:12px; text-align:center; font-size:.8rem; color:var(--text-2);">${r.sickLeave}/${r.personalLeave}/${r.vacationLeave}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
...
$oldEnd = "    </div>`n  </div>`;`n}`n`n  function toggleLeaveBulkMode() {"
$newEnd = "    </div>`n  </div>`}`n  `;`n}`n`n$empeoLogic`n`n  function toggleLeaveBulkMode() {"

$oldEnd2 = "    </div>`r`n  </div>`;`r`n}`r`n`r`n  function toggleLeaveBulkMode() {"
$newEnd2 = "    </div>`r`n  </div>`}`r`n  `;`r`n}`r`n`r`n$empeoLogic`r`n`r`n  function toggleLeaveBulkMode() {"

$content = $content.Replace($oldEnd, $newEnd).Replace($oldEnd2, $newEnd2)

Set-Content js/pages.js $content -Encoding UTF8
Write-Host "Injected render helper"
